...
export default function Navbar() {
  ...
  // --- STYLES ---
  const topItemStyle = "pointer-events-auto text-[#1A1A1A] z-50 flex items-center";
  const accentHover = "hover:text-[#BC4B32] transition-colors duration-300";

  // --- DYNAMIC BACKGROUND CLASSES (FULL WIDTH ADJUSTMENTS) ---
  // Changed: Removed rounded corners, added border-b instead of full border
  const navContainerClasses = isScrolled
    ? "bg-[#F8F7F5]/80 backdrop-blur-md border-b border-white/20 shadow-sm py-3 px-6 md:px-10" // Scrolled: Glass, Bottom Border
    : "bg-transparent border-transparent shadow-none py-6 px-6 md:px-10"; // Top: Transparent, More Padding
  ...
  return (
    <>
      ...
      {/* Removed outer padding (p-4) to allow full width */}
      <motion.div
        variants={{
      ...
      >
        {/* ================= TOP ROW (FULL WIDTH NAVBAR) ================= */}
        {/* Removed rounded-full, now it spans full width */}
        <div className={`flex justify-between items-center w-full transition-all duration-500 pointer-events-auto ${navContainerClasses}`}>
            ...
            <Link to="/" className="block group">
              <img
                ...
                className={`object-contain transition-all duration-500 group-hover:scale-105 ${
                    isScrolled ? "w-16 h-10 md:w-20 md:h-12" : "w-20 h-14 md:w-28 md:h-16"
                }`}
              />
            </Link>
          </div>
          ...
          <div className={topItemStyle}>
            <button
              onClick={toggleMenu}
              className={`text-[#1A1A1A] group flex items-center gap-2 md:gap-3 ${accentHover}`}
            >
              <span className="hidden md:block text-[10px] md:text-xs font-bold tracking-[0.2em] uppercase">
                Menu
              </span>

              <div className="flex flex-col gap-[5px] items-end justify-center">
                <span className="w-6 h-[2px] bg-current group-hover:w-4 transition-all duration-300"></span>
                <span className="w-4 h-[2px] bg-current group-hover:w-6 transition-all duration-300"></span>
              </div>
            </button>
          </div>
        ...
        {/* ================= BOTTOM ROW ================= */}
        {/* Added Padding here explicitly because we removed it from the parent wrapper */}
        {location.pathname === "/" && (
          <motion.div
            animate={{ opacity: isVisible && !isScrolled ? 1 : 0 }}
            className="flex justify-between items-end text-[#1A1A1A] pointer-events-auto px-6 md:px-10 pb-6 md:pb-8"
      ...
      <AnimatePresence>
        {isMenuOpen && (
          ...
          >
            ...
            {/* --- MENU HEADER --- */}
            <div className="flex justify-between items-center px-6 md:px-10 py-6 shrink-0">
              <div className="text-[#666666] text-xs font-mono tracking-widest hidden md:block">
                NAVIGATION INDEX
              </div>
              <div className="md:hidden opacity-50 w-10"></div>
            ...
            <motion.div
              ...
              className="p-6 md:px-10 pb-8 md:pb-12 flex justify-between items-end text-[#666666] text-[10px] md:text-xs font-mono tracking-widest uppercase shrink-0"
            >
